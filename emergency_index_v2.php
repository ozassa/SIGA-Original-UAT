<?php
// VERSÃO DE EMERGÊNCIA V2 - MÁXIMA SIMPLICIDADE
require_once __DIR__ . "/simple_session_config.php";

// Verificações seguras de $_SERVER
$request_uri = $_SERVER['REQUEST_URI'] ?? '/';
$http_host = $_SERVER['HTTP_HOST'] ?? 'localhost';
$https_check = $_SERVER['HTTPS'] ?? 'off';

$is_https = (!empty($https_check) && $https_check !== 'off') ? "https://" : "http://";
$req = explode('/', $request_uri);

$host = $is_https . $http_host . '/';
?>
  ...
  <div id="entradageral">
    <div id="entrada">
      ...
      <form action="src/role/access/Access.php" name="frm" id="frm" method="post" autocomplete="off">
        ...
        <?php 
        $v_erro = 0;
        if (isset($_GET['erro']) && is_numeric($_GET['erro'])) {
            $v_erro = (int)$_GET['erro'];
        }
        ?>
        ...
        <label>Senha</label>
        <input name="password" id="password" type="password" onkeypress="javascript:if (event.keyCode == 13) this.form.submit();" />
        ...
        <button class="botaoagm" type="button" id="validar" onclick="javascript: frm.submit();" onkeypress="javascript:if (event.keyCode == 13) frm.submit();">Entrar</button>
      </form>
