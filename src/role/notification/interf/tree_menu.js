...
var autostyles = new Array('tmenu0text', 'tmenu1text', 'tmenu2text', 'tmenu3text');
...
function TreeNode(title, icon, link, expanded, isDynamic)
{
  ...
  this.n         = new Array();
  this.style     = false;    // cc 2002-10-29
}
...
function drawMenu()// OPTIONAL ARGS: nodes = [], level = [], prepend = '', expanded = false, visbility = 'inline', parentLayerID = null
{
  ...
  for (var i=0; i<nodes.length; i++) {
    ...
    // cc 2002-10-29
    var titleString = nodes[i].title;
    var styleStart  = '';
    var styleEnd    = '';
    if (nodes[i].style == 'auto') {
      styleInx = Math.min(currentlevel, autostyles.length-1);
      styleStart = "<span class='"+autostyles[styleInx]+"'>"
      styleEnd   = '</span>';
    }
    
    output = sprintf('%s<nobr>%s%s%s%s%s%s%s%s</nobr><br></div>',
                      divTag,
		      styleStart,
                      prepend,
