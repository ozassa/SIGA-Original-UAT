<?php 
	...
	If($i_Empresa == 2){
		$id_empresa = 5;
      	...
      	$sqlCount = "SELECT Count(p.i_Inform) as num_parcelas
             FROM Parcela p
             WHERE p.i_Inform = ?";
             
$res = odbc_prepare($db, $sqlCount);

// Garantir que idInform seja inteiro
...
odbc_execute($res, [$idInform]);
	...
	for($parcela = $parc; $parcela <= $num_parcelas; $parcela++){
	    	...
	    	If ($verif == 0 && $VlrUltparc > 0 ){
	    	    ...
	    	    $parcelaLimpa = (int) $parcela;

		// 2020/10/02 - AIP: Inclui o número da fatura (IM_Parcela.i_Parcela) na query

$sqlp1 = "
    SELECT TOP $parcelaLimpa p.*, x.i_Parcela as idP
    FROM [OIM].OIM.dbo.IM_Emissao e
    INNER JOIN [OIM].OIM.dbo.IM_Parcela x
            ON x.i_Emissao = e.i_Emissao
           AND x.t_Parcela = 100
           AND x.d_Cancelamento IS NULL
    LEFT JOIN SBCE.dbo.Inform i
           ON e.n_Empresa = CASE WHEN i.i_Empresa = 1 THEN 1 ELSE 5 END
          AND CAST(e.n_Apolice AS INT) = i.n_Apolice
    LEFT JOIN SBCE.dbo.Parcela p
           ON i.id = p.i_Inform
          AND x.n_Parcela = p.n_Parcela
          AND CAST(e.n_Endosso AS INT) = p.n_Endosso
    WHERE p.t_Parcela = 100
    ORDER BY p.n_Endosso, p.n_Parcela
";

// Executando a primeira consulta
$res1 = odbc_exec($db, $sqlp1);

// Verifica se a primeira consulta foi bem-sucedida
if (!$res1) {
    echo "Erro na execução da primeira consulta: " . odbc_errormsg($db);
    exit;
}

// A primeira consulta foi bem-sucedida, você pode processar $res1 se necessário

// Agora, a segunda consulta com o parâmetro $idInform
$sqlp2 = "
    SELECT TOP 1 z.* 
    FROM (
        SELECT p.*, x.i_Parcela as idP
        FROM [OIM].OIM.dbo.IM_Emissao e
        INNER JOIN [OIM].OIM.dbo.IM_Parcela x
                ON x.i_Emissao = e.i_Emissao
               AND x.t_Parcela = 100
...
// Preparando a segunda consulta
$res2 = odbc_prepare($db, $sqlp2);

// Verifica se a preparação foi bem-sucedida
if (!$res2) {
    echo "Erro ao preparar a segunda consulta: " . odbc_errormsg($db);
    exit;
}

// Passando o parâmetro $idInform

if (!odbc_execute($res2, [$idInform])) {
    echo "Erro ao executar a segunda consulta: " . odbc_errormsg($db);
    exit;
}  
 

// Se a execução for bem-sucedida, continue com a lógica...
			  ...
			  $html .= ' 	
							...
							<div class="rounded">
									  <table style="width: 100%; border: 0; ">
										...
										<tr>
											<td colspan="3" style="width: 100%;"><b>Vigência da apólice:</b> '.utf8_decode($today).'</td>
										</tr>
										<tr>
