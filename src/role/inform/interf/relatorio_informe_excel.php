...
<head>
	...
	header("Content-Disposition: attachment; filename=Dados_informe_" . date("dmY") . "-" . time("His") . ".xls");
	?>

	<title>SIEX &mdash; Sistema Integrado de Exporta&ccedil;&atilde;o &mdash; <?php echo $nomeEmp; ?></title>
...
<body>
	...
	$i_Produto = odbc_result($curv, "i_Produto");
	...
	if ($pagina == 1) {  // Dados cadastrais
		...
		$html .= '<strong>1.1 DADOS CADASTRAIS</strong><div style="clear:both">&nbsp;</div>';
		...
		while (odbc_fetch_row($res)) {
			$html .= ' 
								    ...
								    <table width="100%" border="1"  cellspacing="2">		
										...
										';
			// Consultas protegidas com odbc_prepare e odbc_execute
			$queryProduto = "SELECT * FROM Produto WHERE Situacao = 1 AND i_Produto = ?";
			$stmtProduto = odbc_prepare($db, $queryProduto);
			odbc_execute($stmtProduto, [$i_Produto]);

			$html .= '
<tr>
    <td><strong>Tipo de Produto</strong></td>
    <td>' . htmlspecialchars(odbc_result($stmtProduto, 'Nome'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>';

			// Consultando Moeda
			$queryMoeda = "SELECT * FROM Moeda MO 
               INNER JOIN Produto_Moeda PM ON PM.i_Moeda = MO.i_Moeda 
               WHERE MO.Situacao = 1 AND PM.i_Produto = ?";
			$stmtMoeda = odbc_prepare($db, $queryMoeda);
			odbc_execute($stmtMoeda, [$i_Produto]);

			$html .= '
<tr>
   <td><strong>Tipo de Moeda</strong></td>
   <td>' . htmlspecialchars(odbc_result($stmtMoeda, 'Nome'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>

<tr>
   <td><strong>Cobertura para Juros Mora </strong></td>
   <td>' . (odbc_result($res, 'warantyInterest') == 1 ? 'Sim' : 'N&atilde;o') . '</td>
</tr>';

			// Consultando Setor
			$querySetor = "SELECT * FROM Sector WHERE id = ?";
			$stmtSetor = odbc_prepare($db, $querySetor);
			odbc_execute($stmtSetor, [odbc_result($res, 'idSector')]);

			$html .= '
<tr>
   <td><strong>Natureza da Opera&ccedil;&atilde;o</strong></td>
   <td>' . htmlspecialchars(odbc_result($stmtSetor, 'description'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>
...
<tr>
   ...
   <td>' . htmlspecialchars(strtoupper(odbc_result($res, 'name')), ENT_QUOTES, 'UTF-8') . '</td>
</tr>
...
<tr>
   ...
   <td style="text-align:left">' . formata_string('CNPJ', odbc_result($res, 'cnpj')) . '</td>
</tr>

<tr>
  <td><strong>Inscri&ccedil;&atilde;o Estadual ou Municipal </strong></td>
  <td style="text-align:left">' . formata_string('IE', odbc_result($res, 'ie')) . '</td>
</tr>             

<tr>
   <td><strong>Endere&ccedil;o</strong></td>
   <td>' . htmlspecialchars(odbc_result($res, 'address'), ENT_QUOTES, 'UTF-8') . ', ' . odbc_result($res, 'addressNumber') . '</td>
</tr>        

<tr>                                
   <td><strong>Complemento</strong></td>
   <td>' . htmlspecialchars(odbc_result($res, 'addressComp'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>    

<tr>
   <td><strong>Cidade</strong></td>
   <td>' . htmlspecialchars(odbc_result($res, 'city'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>

<tr>
   <td><strong>UF</strong></td>
   <td>' . odbc_result($res, 'estado') . '</td>
</tr>

<tr>
   <td><strong>Cep</strong></td>
   <td>' . formata_string('CEP', odbc_result($res, 'cep')) . '</td>
</tr>   

<tr>
   <td><strong>Região</strong></td>
   <td>';
      $qry = "SELECT description FROM Region WHERE id = ?";
      $stmt = odbc_prepare($db, $qry);
      odbc_execute($stmt, [odbc_result($res, 'idRegion')]);
      $html .= htmlspecialchars(odbc_result($stmt, 1), ENT_QUOTES, 'UTF-8') .
   '</td>
</tr>
        

<tr>
   <td><strong>E-mail da Empresa</strong></td>
   <td>' . htmlspecialchars(odbc_result($res, 'email'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>

<tr>
   <td><strong>Contato</strong></td>
   <td>' . htmlspecialchars(odbc_result($res, 'contact'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>    

<tr>
   <td><strong>E-mail do Contato</strong></td>
   <td>' . htmlspecialchars(odbc_result($res, 'emailContact'), ENT_QUOTES, 'UTF-8') . '</td>
</tr>
...
<tr>
   ...
   <td>' . formata_string('TEL', odbc_result($res, 'tel')) . '</td>
</tr>

<tr>
   <td><strong>Fax (DDD)</strong></td>
   <td>' . (odbc_result($res, 'fax') ? formata_string('TEL', odbc_result($res, 'fax')) : 'N&atilde;o informado') . '</td>
</tr>
...
<tr>                                                    
 <td><strong>Utilizar Dados acima para cobran&ccedil;a?</strong></td>
 <td>' . (odbc_result($res, 'sameAddress') == 1 ? 'Sim' : 'N&atilde;o') . '</td>
</tr>';

			// Liberação das conexões ODBC
			odbc_free_result($stmtProduto);
			odbc_free_result($stmtMoeda);
			odbc_free_result($stmtSetor);
    ...
    <tr>										
       <td><strong>Endere&ccedil;o</strong></td>
       <td>' . htmlspecialchars(odbc_result($res, 'chargeAddress'), ENT_QUOTES, 'UTF-8') . ', ' . htmlspecialchars(odbc_result($res, 'chargeAddressNumber'), ENT_QUOTES, 'UTF-8') . '</td>
    </tr>		

    <tr>										
       <td><strong>Complemento</strong></td>
       <td>' . htmlspecialchars(odbc_result($res, 'chargeAddressComp'), ENT_QUOTES, 'UTF-8') . '</td>
    </tr>	

    <tr>
       <td><strong>Cidade</strong></td>
       <td>' . htmlspecialchars(odbc_result($res, 'chargeCity'), ENT_QUOTES, 'UTF-8') . '</td>
    </tr>
    
    <tr>
       <td><strong>UF</strong></td>
       <td>' . htmlspecialchars(odbc_result($res, 'chargeUf'), ENT_QUOTES, 'UTF-8') . '</td>
    </tr>

    <tr>
       <td><strong>Cep</strong></td>
       <td>' . formata_string('CEP', odbc_result($res, 'chargeCep')) . '</td>
    </tr>';
			}
			...
			$html .= '
										...
										<tr>																			
													 <td><strong>Setor de Atividade</strong></td><td>' . utf8_encode(odbc_result($rs1, 'description')) . '</td>
													</tr>		
													
													<tr>
													   <td><strong>Principais Produtos</strong></td><td>' . utf8_encode(odbc_result($res, 'products')) . '</td>
													</tr>															
													
													<tr>
													    <td><strong>Prazo M&eacute;dio Usualmente Concedido Para Pagamento: (Dias)</strong></td><td>' . odbc_result($res, 'frameMed') . '</td>
													</tr>															
													<tr>
			...
			if (odbc_result($res, 'associatedCompanies')) {
				$html .= '<tr>
											            <td colspan="2">' . utf8_encode(odbc_result($res, 'companyGroup')) . '
											           </td>
											         </tr>';
			}
			...
			if (odbc_result($res, 'hasAssocCompanies')) {
				$html .= '
												   <tr>
													  <td colspan="2">' . utf8_encode(odbc_result($res, 'associatedCompanies')) . '
													  </td>
												  </tr>';
			}
			...
			if (odbc_result($res, 'hasAnother')) {
				$html .= '<tr><td><strong>Outros Quais?</strong></td><td>' . utf8_encode(odbc_result($res, 'another')) . '</td></tr>';
			}
										   ...
										   <div style="clear:both">&nbsp;</div>
											...
											<table width="100%" border="0" align="center" cellspacing="2">											     										
												  ...
												  <tr> 
													...
													<td width="55%">' . utf8_encode(odbc_result($cur91, "Nome_Responsavel")) . '</td>
												  </tr>
												  
												  <tr>
													<td width="45%">Cargo</td>
													<td width="55%">' . utf8_encode(odbc_result($cur91, "Cargo_Responsavel")) . '</td>
												  </tr>	
												  
												   <tr>
													<td width="45%">A quem se Reporta</td>
													<td width="55%">' . utf8_encode(odbc_result($cur91, "Nome_Reporta")) . '</td>
												  </tr>	
												  
												  <tr>
													<td width="45%">Cargo</td>
													<td width="55%">' . utf8_encode(odbc_result($cur91, "Cargo_Reporta")) . '</td>
												  </tr>	
			...
			if (odbc_result($cur91, "Relacao_PPE") == 1) {
				$html .= '
														  <tr>
															...
															<td width="55%">' . utf8_encode(odbc_result($cur91, "Cargo_PPE")) . '</td>
														  </tr>';
				...
				$html .= '
															 ...
															 <tr>
																...
																<td width="55%">' . utf8_encode(odbc_result($cur91, "Parentesco_PPE")) . '</td>
															 </tr>	';
			}
		...
		$queryj = "SELECT * FROM Volume WHERE idInform = ?";
		$curj = odbc_prepare($db, $queryj);
		odbc_execute($curj, [$_REQUEST["inform"]]);

		if (odbc_fetch_row($curj)) {
			$html .= '
										  <div style="clear:both">&nbsp;</div>
										    <strong>1.3 PROJE&Ccedil;&Atilde;O DE VENDAS</strong>	
											
											
											<table width="100%" border="1" align="" cellspacing="2">											     										
												  <tr>
												    <td></td>
												  ...
												  <tr>
													...
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol1"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol2"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol6"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol8"), 2, ",", ".") . '</td>
												  </tr>
												
												 <tr>
													<td>N&ordm; de Compradores</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol9"), 0, ".", "") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol10"), 0, ".", "") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol14"), 0, ".", "") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol16"), 0, ".", "") . '</td>
											   </tr>										
										  </table>
		...
		$html .= ' <strong>1.4 HIST&Oacute;RICO DE VENDAS</strong>
										  ...
										  <table width="100%" border="1" align="" cellspacing="2">
												  ...
												  <tr>
													...
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol17"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol18"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol22"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol24"), 2, ",", ".") . '</td>
												 </tr>
												  <tr>
													<td width="162">Ano Passado</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol25"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol26"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol30"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol32"), 2, ",", ".") . '</td>
												  </tr>
												  <tr>
													<td width="162">Ano Retrasado</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol33"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol34"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol38"), 2, ",", ".") . '</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol40"), 2, ",", ".") . '</td>
												  </tr>
												  <tr>
													<td colspan="4">(*) N&uacute;mero de Meses</td>
													<td style="text-align:right">' . number_format(odbc_result($curj, "vol41"), 0, "", "") . '</td>
												 </tr>
		...
		$html .= '<div style="clear:both">&nbsp;</div>
										   <strong>1.6 DISTRIBUI&Ccedil;&Atilde;O DE VENDAS POR PA&Iacute;S</strong>';
		...
		$html .= ' <strong>1.7 RELA&Ccedil;&Atilde;O DE CLIENTES PARA AN&Aacute;LISE PRELIMINAR - Buyer Study</strong>
									';
		...
		$qry = "SELECT imp.name, address, city, c.name, tel, prevExp12, numShip12,
							 ...
							 FROM Importer imp
							 JOIN Country c ON (idCountry = c.id)
							 WHERE idInform = ? AND state <> '7' AND state <> 9
		...
		while (odbc_fetch_row($cur)) {
			...
			$html .= '<tr>		   
													...
													<td colspan="3">' . utf8_encode(odbc_result($cur, 1)) . '</td>
												</tr>
												<tr><td  colspan="1">	 
												...
												<tr><td colspan="1">
													 ...
													 ' . utf8_encode(odbc_result($cur, 2)) . '
												</td></tr>
											 
												<tr><td colspan="1"> 
													<strong>Cidade:</strong></td><td colspan="3">
													' . utf8_encode(odbc_result($cur, 3)) . '
												</td></tr>
												<tr><td colspan="1"> 
												   <strong>Pa&iacute;s:</strong></td><td colspan="3">
												   ' . utf8_encode(odbc_result($cur, 4)) . '
												 </td></tr>
												 ...
												 <tr><td colspan="1"> 
													...
													' . utf8_encode(odbc_result($cur, 13)) . '
												 </td></tr>
												 ...
												 <tr><td colspan="1"> 
													...
													' . (odbc_result($cur, 15) ? "Sim" : "N&atilde;o") . '
												 </td></tr>
												 
												  <tr><td colspan="1"> 
													 <strong>Divulga nome ao importador:</strong></td><td colspan="3">
													 ' . (odbc_result($cur, "divulgaNome") == 1 ? "Sim" : "N&atilde;o") . '
												  </td></tr>
												</td>                               
											  </tr>
											  
											  <tr>											 
												<Td align="right"><strong>Prev. Vol. Vendas</strong></Td>
												<Td align="right"><strong>N&uacute;m. de Emb/Ano</strong></Td>
												<Td align="right"><strong>Period. de Emb.</strong></Td>
												<Td align="right"><strong>Prazo de Pagto.</strong></Td>
												  
											  </tr>
												   
											  <tr>
												<TD align="right">' . number_format(odbc_result($cur, 6), 2, ",", ".") . '</TD>
												<TD align="right">' . odbc_result($cur, 7) . '</TD>
												<TD align="right">' . odbc_result($cur, 8) . '</TD>
												<TD align="right">' . odbc_result($cur, 9) . '</TD>
		...
		if ($i == 0) {
			$html .= '<TR>
                                                   <TD align="" colspan=4>Nenhum buyer cadastrado</TD>
                                      </TR>';
		}
		...
		$html .= '<div style="clear:both">&nbsp;</div>
									<div style="clear:both">&nbsp;</div>
								    <strong>1.9 HIST&Oacute;RICO DE PERDAS</strong>';
		...
		if (odbc_fetch_row($cur)) {
			...
			if ($declaracao) {
				$html .= '<strong>Declaro que n&atilde;o houve perda</strong>';
			}
														  ...
														  <table width="100%" border="1" align="" cellspacing="2"> 
															 ...
															 <tr>
															  ...
															  <tr>
																...
																<td align="right">' . number_format(odbc_result($cur, "val1"), 2, ",", ".") . '</td>
																<td align="right">' . number_format(odbc_result($cur, "val2"), 2, ",", ".") . '</td>
																<td align="right">' . number_format(odbc_result($cur, "val3"), 2, ",", ".") . '</td>
															  </tr>
															  <tr>
		...
		$html .= '<div style="clear:both">&nbsp;</div>
									         <strong>1.5 DISTRIBUI&Ccedil;&Atilde;O DE VENDAS A PRAZO POR FAIXA DE LIMITE DE CR&Eacute;DITO</strong>
		...
		while (odbc_fetch_row($cur)) {
			...
			$html .= '</td>
																    ...
																    <td align="right" style="text-align:right">' . number_format((odbc_result($cur, 'n_Clientes') / $total1) * 100, 2, ',', '.') . '%' . '</td>
																    <td align="right" style="text-align:right">' . number_format(odbc_result($cur, 'v_Valor'), 2, ',', '.') . '</td>
																    <td align="right" style="text-align:right">' . number_format((odbc_result($cur, 'v_Valor') / $total3) * 100, 2, ',', '.') . '%' . '</td>
															      </tr>';
		...
		$html .= '<strong>1.10 DETALHAMENTO DAS PERDAS EFETIVAS POR FAIXA DE VALOR</strong>
		...
		if ($i_Produto == 2) {
			$html .= '<div style="clear:both">&nbsp;</div><strong>1.4 DISTRIBUI&Ccedil;&Atilde;O DE VENDAS POR TIPO E CANAL</strong>';
		} else {
			$html .= '<div style="clear:both">&nbsp;</div><strong>1.4 DISTRIBUI&Ccedil;&Atilde;O DE VENDAS POR CANAL E REGI&Otilde;ES</strong>';
		}
        ...
        WHERE Inf.id = ? 
          AND (VTC.Situacao = 0 OR IVTC.v_Valor IS NOT NULL)
		...
		while (odbc_fetch_row($cur)) {
			if (odbc_result($cur, 'Tipo_Venda') != $tipovenda) {
				...
				$html .= '  										     
											                    <table width="100%" border="1" cellspacing="2" style="width:450px;">';


				if ($i == 0) {
					$html .= '<tr>
															   <td width="50%"><strong>' . utf8_encode(odbc_result($cur, "Tipo_Venda")) . '</strong></td>
			...
			if ($i == 1) {
				$html .= '<tr>
													    <td>' . utf8_encode(odbc_result($cur, 'Descricao')) . '</td>
													    <td style="text-align:right">' . number_format(odbc_result($cur, 'v_Valor'), 2, ',', '.') . '</td>
													    <td style="text-align:right">' . number_format((odbc_result($cur, 'v_Valor') / $total1 * 100), 2, ',', '.') . '</td>                   
												       </tr>';
			} else if ($i == 2) {
				$html .= '<tr>
														<td>' . utf8_encode(odbc_result($cur, 'Descricao')) . '</td>
		...
		$html .= '<strong>1.8 D&Iacute;VIDAS VENCIDAS</strong>';
        ...
        WHERE Inf.id = ? AND (DV.Situacao = 0)
		...
		while (odbc_fetch_row($cur)) {
			...
			if ($i == 1) {
				$html .= '<tr>
														...
														<td>' . number_format((odbc_result($cur, 'v_Valor') / $total3 * 100), 2, ',', '.') . '</td>                   
												   </tr>';
			} else if ($i == 2) {
				$html .= '<tr>
														<td>' . odbc_result($cur, 'Descricao') . '</td>
	...
	$html .= '
									   ...
									   <div style="clear:both">&nbsp;</div>
										...
										<div id="cobtexto">
										   As informa&ccedil;&otilde;es prestadas neste question&aacute;rio autorizam a elabora&ccedil;&atilde;o de propostas de 
										   seguros e ser&atilde;o tratadas com confidencialidade. N&atilde;o constituem qualquer compromisso de subscri&ccedil;&atilde;o de ap&oacute;lice. 
										   Caso haja a contrata&ccedil;&atilde;o do seguro, o presente question&aacute;rio ser&aacute; parte integrante da ap&oacute;lice.															
