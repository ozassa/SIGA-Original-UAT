...
if (odbc_fetch_row($cur)) {
  ...
  $r = odbc_exec ($db, "UPDATE Inform SET state = 9 WHERE id = $idInform");
  ...
  if ($r) {
    //      if ($abc > 1000000)
    $r = $notif->newInfCredito($userID, $name, $idInform, $db);
...
//Log do Analise de Crédito ( Cancelamento informe )
   ...
   if (odbc_exec($db, $sql) ) {
    	...
    	$sql = "Insert Into Log_Detalhes (id_Log, campo, valor, alteracao) ".
               "values ('$cur', 'Informe', 'Cancelamento', 'Alteração')";
		$rs = odbc_exec($db, $sql);
