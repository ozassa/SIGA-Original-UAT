...
<input type="hidden" name="comm" value="accept">
...
<?php  if(odbc_fetch_row($cur)){
  ...
  //$ChangeCredit	     = number_format(odbc_result($cur, 7), 2, ",", ".");
  //$ImporterLimCredit = number_format(odbc_result($cur, 5), 2, ",", ".");

/*   $ChangeStateDate   = odbc_result($cur, 8); */
/*   $ChangeStateDate   = */
/*     substr($ChangeStateDate, 8, 2). "/". */
/*     substr($ChangeStateDate, 5, 2). "/". */
/*     substr($ChangeStateDate, 0, 4); */
/*   $i++; */
}else{
  $c = odbc_exec($db, "select I.name, I.c_Coface_Imp, C.code from Importer I join Country C
                       on I.idCountry=C.id where I.id=$idBuyer");
  $nameBuyer = odbc_result($c, 1);
  $ImporterCoface = odbc_result($c, 2);
  $CodeCountry = odbc_result($c, 3);
}
?>
...
?>
	...
	<input type=hidden name=nameBuyer value="<?php echo  $nameBuyer;?>">
	<input type=hidden name=ImporterCoface value="<?php echo  $ImporterCoface;?>">
	<input type=hidden name=decision_date value="<?php echo  $decision_date;?>">
	<input type=hidden name=CodeCountry value="<?php echo  $CodeCountry;?>">
...
<?php  $q = "SELECT Comment.text FROM Comment WHERE state='1' AND idImporter=$idBuyer";
$c = odbc_exec($db, $q);
if(odbc_fetch_row($c)){
  $obs = odbc_result($c, 1);
}
