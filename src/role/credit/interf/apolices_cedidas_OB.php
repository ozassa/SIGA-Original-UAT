...
if ($excel=="yes") {
  ...
  header('Content-Disposition: attachment; filename="apolices_cedidas_OB.xls"');
}
...
FROM         Inform i INNER JOIN
                      CDOB bb ON i.id = bb.idInform INNER JOIN
                      Agencia a ON bb.agencia = a.id INNER JOIN
                      Banco b ON a.idBanco = b.id
WHERE     (b.tipo = 3) AND (bb.status = 2) AND (i.state = 10)";
...
echo "Total de Apólices Cedidas para o outros Bancos:".$total."<br>";
...
echo "</table><br>";
?>
<form name="frm_view_rel_apolice" action="view_apolices_cedidas_OB.php" method="post" target="_self">
