<?php 
  ...
  if (odbc_fetch_row ($cur)) {
    ...
    $percComissao = number_format(odbc_Result($cur, 'p_Comissao') ?? 0,2,',','.');
  }
  ...
  if ($tipdve == "1") {
	...
	$extnas = $mo." ".number_format($nas ?? 0, 2, ',','.')." (".strtolower($numberExtensive->extensive(number_format($nas ?? 0,2, '.',''),$currency)).')';
  ...
  $mod_b2604                 = number_format(odbc_result($cur4,'b2604Perc') ?? 0,2,',','.');
  $b2604NivelMax             = number_format(odbc_result($cur4,'b2604NivelMax') ?? 0,2,',','.');
  $b2604NivelMaxExt          = $numberExtensive->extensive(number_format(odbc_result($cur4,'b2604NivelMax') ?? 0,2,'.',''),$currency);
  
  $b2801NivelMax             = number_format(odbc_result($cur4,'b2801NivelMax') ?? 0,2,',','.');
  $b2801NivelMaxExt          = $numberExtensive->extensive(number_format(odbc_result($cur4,'b2801NivelMax') ?? 0,2,'.',''),$currency);
  $b2801Perc                 = number_format(odbc_result($cur4,'b2801Perc') ?? 0,2,',','.');
  
  $b404NivelMax              = number_format(odbc_result($cur4,'b404NivelMax') ?? 0,2,',','.');
  $b404NivelMaxExt           = $numberExtensive->extensive(number_format(odbc_result($cur4,'b404NivelMax') ?? 0,2,'.',''),$currency);
  $mod_b404                  = number_format(odbc_result($cur4,'b404Perc') ?? 0,2,',','.');
  $riscopolitico             = odbc_result($cur4,'Texto_Risco_Politico');
  
  $d201                      = number_format(odbc_result($cur4,'d201') ?? 0,2,',','.');
  $valorExtD201              = $numberExtensive->extensive(number_format(odbc_result($cur4,'d201') ?? 0,2,'.',''),$currency);
  $f401NivelSinistralidade   = number_format(odbc_result($cur4,'f401NivelSinistralidade') ?? 0,2,',','.');
  $f401PercPremio            = number_format(odbc_result($cur4,'f401PercPremio') ?? 0,2,',','.');
  $mod_b802                  = number_format(odbc_result($cur4,'b802') ?? 0,2,',','.');
          ...
          $msg11d = "";
		   ...
		   $html_mod = "<table class='tbl_mods'>";
            ...
            $cury = odbc_exec ($db, $query);
						...
						$nivel = strtolower($numberExtensive->extensive(number_format($nivel_d602 ?? 0,2, '.',''),$fMoeda ?? 0));
						$d602 = "O nível de pequenos sinistros é de: ".$mo." ".number_format($nivel_d602 ?? 0,2,',','.')." (".$nivel." ". strtolower($ext).")";
						$html_mod .= '
							<tr>
								<td  colspan="1" width="8%">&nbsp;</td>
								<td  colspan="1" width="8%">&nbsp;</td>
