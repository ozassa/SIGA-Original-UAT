<?php  $cookie = $field -> getField ("cookie");
	...
	$cur = odbc_exec ($db, "SELECT ChangeAddress.address, ChangeAddress.tel,
					ChangeAddress.name, ChangeAddress.city, Country.name, ChangeAddress.stateDate
	...
	if (odbc_fetch_row ($cur)) {
		$newAddress 	= odbc_result($cur, 1);		
		$newTel		= odbc_result($cur, 2);		
		$newName 	= odbc_result($cur, 3);		
		$newCity	= odbc_result($cur, 4);		
		$newCountry 	= odbc_result($cur, 5);
		$stateDate 	= odbc_result($cur, 6);				
	}
