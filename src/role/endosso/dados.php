...
$c = odbc_exec($db, "select bornDate, solicitante, idUser from Endosso where id=$idEndosso and state=1");
if(odbc_fetch_row($c)){
...
$c = odbc_exec($db,
        ...
        e.number, e.addresscomp, e.numberOld, e.addresscompOld from EndossoDados e join Region r1 on e.idRegion=r1.id join Region r2 on e.idRegionOld=r2.id where e.idEndosso=$idEndosso");
if(odbc_fetch_row($c)){
