...
//error_log("Conteúdo de \$_REQUEST: " . print_r($_REQUEST, true));
//extract($_REQUEST);

$comm = isset($_REQUEST['comm']) ? trim($_REQUEST['comm']) : null;
$comm = $comm !== null ? htmlspecialchars($comm, ENT_QUOTES, 'UTF-8') : null;

$status = isset($_REQUEST['status']) ? trim($_REQUEST['status']) : null;
$status = $status !== null ? htmlspecialchars($status, ENT_QUOTES, 'UTF-8') : null;
...
if ($comm == 'cessaoBB') {
  //$q = "SELECT tipo FROM Banco WHERE id = $idBanco";
  //$ban = odbc_exec($db, $q);
  ...
  odbc_execute($stmt, array($idBanco));

  $tipoBanco = odbc_result($stmt, 1);
...
} else if ($comm == 'consultaCessao' && $perfilUsuario == 'B') {
  $title = 'Cessão de Direito';
...
} else if ($comm == 'cadBanco' && $perfilUsuario == 'F') {
  $title = 'Banco';
...
} else if ($comm == 'cadAg' && $perfilUsuario == 'F') {
  $title = 'Agência';
...
} else if ($comm == 'emiteCessaoDireito' && $perfilUsuario == 'B') {
  $title = 'Cessão de Direito - Emissão de Cessão de Direito';
...
} else if ($comm == 'emiteCessaoDireitoSeguradora' && $perfilUsuario == 'F') {
  $title = 'Cessão de Direito - Emissão de Cessão de Direito';
...
} else if ($comm == 'cancelaCessaoDireitoBB' && $perfilUsuario == 'B') {
  $title = 'Cessão de Direito - Cancelamento de Cessão de Direito';
...
} else if ($comm == 'cancelaCessaoDireitoSeguradora' && $perfilUsuario == 'F') {
  $title = 'Cessão de Direito - Cancelamento de Cessão de Direito';
