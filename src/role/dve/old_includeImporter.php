...
if(check_dates($dataEmb, $dataVenc)){
  $valorEmb = conserta($valorEmb);
  $proex = conserta($proex);
  $ace = conserta($ace);
  if($proex + $ace > $valorEmb){
    $msg = "A soma dos valores de PROEX e ACE nao deve ser maior que o valor embarcado";
  }else{
    $excluido = 0;
    ...
    if($idDetail){
      ...
      $x = odbc_exec($db,
		     ...
		     "fatura='$fatura', totalEmbarcado='" . (str_replace (",", ".", $valorEmb)) . "', proex='" . (str_replace (",", ".", $proex)). "', ace='" . (str_replace (",", ".", $ace)) . "',
	 ...
	 // die();
      ...
      $cquery = "insert into DVEDetails (idDVE, idImporter, idCountry, embDate, ".
		     ...
		     "', '". dmy2ymd($dataVenc). "', '$fatura','" . (str_replace (",", ".", $valorEmb)) . "' , '" . (str_replace (",", ".", $proex)). "', ".
      ...
      $y = odbc_exec($db,
		     ...
		     "', '$fatura', '" . (str_replace (",", ".", $valorEmb) ) . "', '" . (str_replace (",", ".", $proex)) . "', '" . (str_replace (",", ".", $ace)) . "', $modalidade, $userID, 1, getdate(), $lastId)");
    }
