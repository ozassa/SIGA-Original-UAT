...
CREATE PROCEDURE ssA_PagRec_Sin_Res
	@i_Aviso		int
AS
...
UPDATE 	PagRec
SET	v_Documento = case when AV.v_Sinistro * (PR.p_Cobertura/100) * (1-(RE.p_Seguradora/100)*(RE.p_Resseguro/100)) < RE.v_Max_Retencao
...
JOIN	PagRec PA	ON	AV.i_Aviso = PA.i_Aviso AND RE.c_Seg = PA.c_Seg
WHERE	PA.i_Aviso = @i_Aviso AND PA.t_Doc = 7 AND PA.s_Pagamento = 0
