<?php $q = "UPDATE Sinistro SET status = 6 WHERE id = $idSinistro";
	...
	if (!$cur) {
		$msg = "Problemas na atualização da base";
             ...
             if ($r){
                 $r = $notif->sinistroNaoAceito($idNotification, $idImporter, $idInform, $idInform, $db, $name, $idSinistro, $nameI);
                 if (!$r) {
             ...
             $q = "INSERT INTO SinistroObs (idSinistro, name, date, obs) VALUES ($idSinistro, '$user->name', getdate(),  'Sinistro não Aceito')";
  	     $obs = odbc_exec($db, $q);
       	}
