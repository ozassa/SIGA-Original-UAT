<?php $q = "UPDATE Sinistro SET status = 4 WHERE id = $idSinistro";
	...
	if (!$cur) {
		$msg = "Problemas na atualização da base";
             ...
             if ($r){
                 $r = $notif->sinistroSuspenso($idNotification, $idImporter, $idInform, $idInform, $db, $name, $idSinistro, $nameI);
                 if (!$r) {
             ...
             $q = "INSERT INTO SinistroObs (idSinistro, name, date, obs) VALUES ($idSinistro, '$user->name', getdate(),  'Sinistro Suspenso')";
  	     $obs = odbc_exec($db, $q);
       	}
